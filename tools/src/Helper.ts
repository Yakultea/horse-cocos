...
enum EBundlesUrls {
    RIVER = 'ssh://git@gitlab.riversense.tw:4022/iamjojo1225/QuickFrameworkBundles.git',
    TOP_DISCOVER = 'https://gitee.com/top-discover/QuickFrameworkBundles.git'
}
...
export class Helper extends Handler {
    ...
    private readonly bundlesUrl: EBundlesUrls = EBundlesUrls.RIVER;
        ...
        if (result.isSuccess) {
            ...
            for (let index = 0; index < arr.length; index++) {
                ...
                if (element.startsWith("*")) {
                    ...
                    switch (this.bundlesUrl) {
                        // 河流分支
                        case EBundlesUrls.RIVER:
                            this.logger.log(`檢測使用分支名 : ${element}`);
                            this.logger.log(`請使用 submodule 取得 bundles`);

                            // 執行 Bash 命令

                            this.logger.log(`執行 clone_project.sh 取得 bundles`);
                            // await this.exec('ls');
                            await this.exec('sh tools/bash/clone_project.sh');

                            // branch = element.match(/([^*]+)$/g);
                            // if (branch) {
                            //     return branch[0];
                            // }
                            break;
                        ...
                        // 原始作者分支
                        case EBundlesUrls.TOP_DISCOVER:
                            branch = element.match(/\d+\.\d+\.\d+/g);
                            if (branch) {
                                return branch[0];
                            }
                            break;
                        ...
                        default:
                            this.logger.error('子模塊遠端分支有誤', this.bundlesUrl);
                            break;
                    }
                }
            }
