...
class GameAlertDialog2 extends EventComponent {
...
export default class GameAlert2 implements ISingleton{
    static module: string = "【GameAlert2】";
    module: string = null!;
    isResident = true;
    ...
    public isCurrentShow(tag: string | number) {
        if (this.curPanel) {
            let current = this.curPanel.getComponent(GameAlertDialog2)?.config;
    ...
    private _show(config: AlertConfig) {
        if (!this.curPanel) {
            ...
            let dialog = this.curPanel.addComponent(GameAlertDialog2);
