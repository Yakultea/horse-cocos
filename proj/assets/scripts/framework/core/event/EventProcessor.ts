...
export interface EventAgrs {
     ...
     * @description 绑定事件类型
     */
    bind: "Default" | "Dispatcher" | "Game" | "Input" | "Node";
    /**@description 事件类型名 */
...
export interface IEventProcessor {
    ...
    /**
     * @description 註冊隻回響一次的事件
     * @param eventName
     * @param callback 回調
     * @param target target
     */
    // eventName: string, func: EventCallback
...
export class EventProcessor implements IEventProcessor {
    ...
    _on(args: EventAgrs): void {
        switch (args.bind) {
            case "Default": this._onDefault(args); break;
    ...
    _off(args: EventAgrs): void {
        switch (args.bind) {
            case "Default": this._offDefault(args); break;
    ...
    private _onDefault(args: EventAgrs) {
        if (!args.target) {
            args.target = this;
        }
        ...
        App.dispatcher.on(args.type, args.cb!, args.target, args.once);
        this._events.set(args.type, args);
    }

    private _offDefault(args: EventAgrs) {
        if (!args.target) {
            args.target = this;
        }
        if (this._events.has(args.type)) {
            //事件移除
            App.dispatcher.off(args.type, args.target);
            //删除本地事件
