 ...
 * @description 快照節點
 * 注意，只會截圖快照攝像頭下的可見節點
 * 如果需要拍照全部分，請設定screenShotCamera的Visibility
 * 
 * 目前有個bug未解決，當螢幕大小比例比設計解析度窄時，截圖底部會被截斷一部分,暫時未找到原因
 * @example
 * ```ts
...
export class Snapshot extends Component {
    ...
    /**@description 截圖完成,除錯時用來檢查截圖是否正確 */
    ...
    private savaAsImage(width: number, height: number, arrayBuffer: Uint8Array) {
        if (sys.isBrowser) {
            ...
            //原生win32平臺排程執行會造成崩潰，請直接用release模式開啟，可正常工作
            let date = new Date()
