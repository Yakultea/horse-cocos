...
class HelperImpl extends Helper {
    ...
    addHotAddress(url: string) {
        if (this.data && this.data.historyIps.indexOf(url) == -1) {
            ...
            this.logger.log(`${this.module}{新增歷史記錄 :${url} 成功`);
            return true;
        }
        return false;
    }
    /**@description 刪除不包含在包內的bundles */
    async onDelBundles() {
        const config = {
            title: '警告',
            detail: '',
        ...
        const code = await Editor.Dialog.info('執行此操作將會刪除不包含在包內的所有bundles,是否繼續？', config);
        if (code.response == 1) {
    ...
    updateToConfigTS() {
        ...
        if (existsSync(configTSPath)) {
            ...
            let replace = function () {
                self.logger.log(`${self.module}更新熱更新地址為:${serverID}`);
            ...
            let replaceAutoVersion = function () {
                self.logger.log(`${self.module}更新是否使用了自動版本:${self.data?.isAutoVersion}`);
            ...
            this.logger.error(`${this.module}${configTSPath}不存在，無法重新整理配置到程式碼`);
        }
    }
